...
import org.toint.test.constants.TointTestConstants;
import org.toint.test.orders.AcceleratorTestOrderData;
 ...
 */
@SystemSetup(extension = TointTestConstants.EXTENSIONNAME)
public class TestDataSystemSetup extends AbstractSystemSetup
{
	...
	public void createProjectData(final SystemSetupContext context)
	{
		if (getBooleanSystemSetupParameter(context, CREATE_TEST_DATA))
		{
			...
			importImpexFile(context, "/tointtest/import/qa-test-users.impex");

			LOG.info("Creating Reviews...");
			importImpexFile(context, "/tointtest/import/reviews.impex");

			LOG.info("Creating Test Payment Subscriptions...");
			getAcceleratorTestOrderData().createPaymentInfos();

			LOG.info("Creating CS Tickets...");
			importImpexFile(context, "/tointtest/import/cstickets.impex");
			...
			importImpexFile(context, "/tointtest/import/csquotes.impex");
		}
	}
}
